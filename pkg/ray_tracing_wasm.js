...
*/
export function draw(ctx, json, width, height, num_samples, max_bounces) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passStringToWasm0(json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        wasm.draw(retptr, addBorrowedObject(ctx), ptr0, len0, width, height, num_samples, max_bounces);
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('ray_tracing_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
